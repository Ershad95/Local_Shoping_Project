...
namespace Presentation
{
    public partial class Chart : Form
    {
        ...
        {
            OracleConnection connection = (new DataAccess().connection);
            connection.Open();
            OracleCommand cmd = new  OracleCommand("select * from factor_froosh", connection);
            OracleCommand cmd2 = new OracleCommand("select * from factor_kharid", connection);
            OracleCommand cmd3 =new OracleCommand("select  factor_froosh.price , moshtari.name , FACTOR_FROOSH.FACTOR_DAY  from moshtari,factor_froosh where  moshtari.id = factor_froosh.Idmoshtari", connection);
            OracleCommand cmd4 =new OracleCommand("select FACTOR_KHARID.PRICE , FACTOR_FROOSH.PRICE from FACTOR_FROOSH,FACTOR_KHARID where FACTOR_FROOSH.IDKALA = FACTOR_KHARID.IDKALA",connection);

            OracleCommand cmd5 =new OracleCommand("select kala.name,kala.PRICE,FACTOR_KHARID.COUNT from kala,FACTOR_KHARID where kala.ID = FACTOR_KHARID.IDKALA", connection);
            ...
            while (reader4.Read())
            {
                ...
                //PEC[index] = (sell[index] - buy[index])/buy[index]*100;
